...
class OurWork extends \yii\db\ActiveRecord
{
    ...
    public static function getList($url, $filter, $limit, $ofsset){
        ...
        $list = MenuCategory::getListIDFromMenu('our-work');
        ...
        $query = "
            SELECT mc.slug AS 'type', mc.short, uc.info_user_id, IFNULL(
            (
                SELECT src
                FROM user_media
            ...
            " WHERE (mc.id IN ({$list}) OR mc.parent_id IN ({$list}))
            AND IF(:url = '', true, 
        ...
        $query = "
            SELECT ow.type, ow.info_user_id, (
                SELECT src
                FROM user_media
                WHERE info_user_id = ow.info_user_id
                AND `type` = 'image'
                ORDER BY `order` ASC
                LIMIT 1
            ) AS 'logo'
            FROM our_work ow
            LEFT JOIN user_info ui ON ui.id = ow.info_user_id
            WHERE IF(:type = '', true, ow.type = :type)
