...
//        $contentMail .= 'Subject: =?' . $this->smtp_charset . '?B?'  . base64_encode($subject) . "=?=\r\n";
        ...
        try {
            if(!$socket = @fsockopen($this->smtp_host, $this->smtp_port, $errorNumber, $errorDescription, 30)){
                throw new NotFoundHttpException($errorNumber.".".$errorDescription, 404);
            }
            if (!$this->_parseServer($socket, "220")){
                throw new NotFoundHttpException('Connection error', 404);
            }
            ...
            if (!$this->_parseServer($socket, "250")) {
                fclose($socket);
                throw new NotFoundHttpException('Error of command sending: HELO', 404);
            }
            
            fputs($socket, "AUTH LOGIN\r\n");
            if (!$this->_parseServer($socket, "334")) {
                fclose($socket);
                throw new NotFoundHttpException('Autorization error', 404);
            }
            ...
            if (!$this->_parseServer($socket, "250")) {
                fclose($socket);
                throw new NotFoundHttpException('Error of command sending: MAIL FROM', 404);
            }
            ...
            if (!$this->_parseServer($socket, "250")) {
                fclose($socket);
                throw new NotFoundHttpException('Error of command sending: RCPT TO', 404);
            }
            
            fputs($socket, "DATA\r\n");     
            if (!$this->_parseServer($socket, "354")) {
                fclose($socket);
                throw new NotFoundHttpException('Error of command sending: DATA', 404);
            }
            
            fputs($socket, $contentMail."\r\n.\r\n");
            if (!$this->_parseServer($socket, "250")) {
                fclose($socket);
                throw new NotFoundHttpException("E-mail didn't sent",404);
            }
