...
use app\modules\admin\models\DivaUsersSearch;
...
class ProductionUserController extends Controller
{
    ...
    public function actions() {
        return [
            'sorting' => [
                ...
                'query' => \app\models\DivaUser::find(),
            ],
        ];
    }
    ...
    {    
        $searchModel = new DivaUsersSearch();
        $searchModel->target = 'production';
        ...
        if (Yii::$app->request->post('hasEditable')) {
            ...
            $diva_media_id = Yii::$app->request->post('diva_media_id');
            
            $divaMedia = \app\models\DivaMedia::findOne($diva_media_id);
            
            $model = \app\models\DivaUser::findOne([
                'info_user_id'=> $id
            ]);
            
            if($model == null){
                $model = new \app\models\DivaUser();
            ...
            if($diva_media_id == '0'){
                ...
                $model->diva_media_id = $diva_media_id;
                
                if ($model->save(false)) {
                    echo  \yii\helpers\Json::encode(['output' => $divaMedia['title'], 'message' => '']);
                }else{
