...
class ClientsController extends Controller
{
    ...
    {
        $request = Yii::$app->request;
        $model = \app\models\Content::findOne(['type'=>'clients', 'target_id'=>1]);
        if($model== null){
    ...
    public function saveFile($model){
        if(!empty($_FILES['Clients']['name']['file'])){
            $upload = new \app\models\File();
            $upload->file = \yii\web\UploadedFile::getInstanceByName("Clients[file]");
       
            $path = \Yii::getAlias("@webroot/images/clients/");
