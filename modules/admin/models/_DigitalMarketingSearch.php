 ...
 */
class _DigitalMarketingSearch extends UserInfo
{
    ...
    {
        return [
            [['user_id', 'category_id', 'birth', 'address', 'weight', 'collar', 'chest', 'waist', 'hips', 'shoe', 'servCatName', 'add_management'], 'integer'],
    ...
    public function search($params)
    {
        ...
        $query->select([
            ...
            "ui.visa_status",
            "ui.specialization",
            "dm.type AS add_management",
        ]);
        $query->from('user_info ui');
        $query->leftJoin('digital_marketing dm', 'dm.info_user_id = ui.id');
        ...
        if($this['add_management'] != NULL){
            $query->andWhere(['dm.id' => ($this['add_management'] == '0') ? NULL : $this->add_management]);
        }
